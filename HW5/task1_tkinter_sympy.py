...
# Радиус точек для рисования
RADIUS_POINTS = 3
# Цвет точек для рисования
COLOR = 'blue'
# Легковой автомобиль за 100 сантиметров при габаритах в 475 сантиметров 
# максимум может повернуть на 12 градусов
# наши машинки имеют длину 200 точек, следовательно они на такое способны за 42 точки
STEP = 31
MAX_ANGEL = round(STEP / 42.0 * math.pi / 15.0, 8)
# Число точек в эпсилон опресности
EPS = 30
...
def collides(position, obstacle):
    return get_polygon_from_position(position) \
        .intersection(get_polygon_from_obstacle(obstacle))


class Window():
    '''================= Your Main Function ================='''
    def go(self, event):
        self.reset(self.save_plot_path)
        self.angel = MAX_ANGEL
        self.step = STEP
        self.path = self.get_path([self.get_start_position()])
        self.save_plot_path = self.plot_path(self.path)

    def reset(self, plotly_path):
        for point in plotly_path:
            self.canvas.delete(point)

    def plot_path(self, path):
        points = []
        for point in path:
            points.append(self.canvas.create_oval(
                point[0] - RADIUS_POINTS,
                point[1] - RADIUS_POINTS,
                point[0] + RADIUS_POINTS,
                point[1] + RADIUS_POINTS,
                fill=COLOR,
            ))
    ...
    def get_path(self, path):
        ...
        heapq.heappush(heap_list, (
            self.min_distance(path[-1]),
            (len(path) - 1) * self.step,
            path
        ))

        while self.min_distance(path[-1]) > EPS:
            _, _, path = heapq.heappop(heap_list)

            for new_positions in self.list_new_positions(path[-1]):
                x, y, yaw = new_positions
                if (x, y, yaw)  in history:
                    continue
                history.add((x, y, yaw))
                    
                new_path = path.copy()
                new_path.append(new_positions)
                
                heap = (len(new_path) - 1) * self.step
                heapq.heappush(heap_list, (
                    heap + self.min_distance(new_positions),
                    heap,
                    new_path
                ))
            self.reset(self.plot_path(path))
        return path

    def arc_length(self, hord, vect_hord, yaw):
        vect_tangent = np.array([math.sin(yaw), -math.cos(yaw)])
        alfa_position = np.arccos((vect_hord @ vect_tangent) / hord / norm(vect_tangent))
        if alfa_position < 10e-8:
            distance = hord
        else:
            distance = hord * alfa_position / math.sin(alfa_position) 
    ...
    def min_distance(self, position): 
        ...
        vect_hord = np.array([target_x - x, target_y - y])
        hord =  norm(vect_hord)

        distance1 = self.arc_length(hord, vect_hord, target_yaw)
        distance2 = self.arc_length(hord, vect_hord, yaw)

        distance = 0.5 * (distance1 + distance2) + abs(distance1 - distance2)
        return  distance

    def list_new_positions(self, position):
        x, y, yaw = position
        
        new_points = []
        for angle in [-self.angel, 0.0, self.angel]:
            yaw_new = round(yaw + angle, 8)
            x_new = round(x + self.step * math.sin(yaw_new), 8)
            y_new = round(y - self.step * math.cos(yaw_new), 8)
            ...
            if collision:
                continue
            new_points.append([x_new, y_new, yaw_new])
        return new_points
